...
#include "kfx/renderer/software/SwDrawTarget.h" // SwTargetScanline/ScreenHeight (background cache)
...
TbBool parchment_copy_background_at(const struct TbRect *bkgnd_area, int units_per_px)
{
    ...
    // Do the drawing
    const int scanline = SwTargetScanline();
    const int nlines = SwTargetScreenHeight();
    ...
    // The cache is a copy of the software backend's frame buffer: only that
    // backend draws into lbDisplay.WScreen, synchronously, when asked to present
    // an image. Under any other backend the present is a GPU upload and there is
    // nothing here to keep, so every frame goes the direct way.
    const TbBool cacheable = (RendererGetActiveType() == RENDERER_SOFTWARE) && (lbDisplay.WScreen != NULL);
    if (cacheable && (parchment_bg_cache != NULL) && (parchment_bg_cache_len == buf_len)
    ...
    } else
    {
        ...
        d.dst_w = dst_width;        d.dst_h = dst_height;
