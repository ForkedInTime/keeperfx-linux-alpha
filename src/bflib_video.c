...
TbResult LbScreenSwap(void)
{
    ...
    if (lbUseGLPresent) {
        if (ret == Lb_SUCCESS) {
            ...
            if ((lbDrawSurface != NULL) && (SDL_GetSurfacePalette(lbDrawSurface) != NULL)) {
                ...
                const SDL_Palette *pal = SDL_GetSurfacePalette(lbDrawSurface);
    ...
    unsigned char *palette, short buffers_count, TbBool wscreen_vid)
{
    ...
    // sdlFlags is gone with the platform seam; windowFlags is the same information in
    ...
    // we fall back to the legacy CPU blit path below.
    {
        ...
        // and an indexed surface no longer gets a palette allocated for it, so one
        // has to be attached explicitly or every palette write lands nowhere.
        SDL_Surface* glDrawSurface = SDL_CreateSurface(mdinfo->Width, mdinfo->Height, SDL_PIXELFORMAT_INDEX8);
        if (glDrawSurface != NULL && !SDL_CreateSurfacePalette(glDrawSurface)) {
            ERRORLOG("Can't create palette for engine surface (mode %d, %s): %s", (int)mode, mdinfo->Desc, SDL_GetError());
...
/*  // Getting the palette in SDL way may sometimes lead to problems.
    ...
    colours_num = SDL_GetSurfacePalette(lbDrawSurface)->ncolors;
    ...
    srcColors = SDL_GetSurfacePalette(lbDrawSurface)->colors;
    destColors = palette;
    ...
    unsigned short bpp, unsigned long flags)
{
    ...
    else
    {
        ...
        {
            // Lb_VF_BORDERLESS, not Lb_VF_DESKTOP, is what separates these two.
            // BOTH desktop modes carry Lb_VF_DESKTOP -- it means "at whatever the
            // desktop resolution happens to be", not "do not change the video
            // mode" -- so keying on it collapses DESKTOP_FULL into DESKTOP and
            // silently costs it the real mode switch keeperfx.cfg advertises.
            // This is the same fault we fixed before the refactor; upstream's
            // rewrite reintroduced it, so it is fixed again here in their shape.
...
#ifndef _WIN32
    // Every window this fork creates has to be able to host the GL 3.3 context the
    // GPU present backend attaches; SDL3 only accepts that at creation time.
    mdinfo->windowFlags |= KFX_WF_OPENGL;
#endif
